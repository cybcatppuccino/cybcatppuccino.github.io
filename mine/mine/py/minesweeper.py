...
def ms_make_safe_move():
    ...
    # 不再检查 _LOST/_WON，因为这只是尝试一个动作，不是检查全局状态
    if _GAME is None: return {"move":None,"newly":[],"ai_mines":[c for c in _AI.mines],"lost":False,"won":False,"stuck":False,"revealed_count":0}
    mv = _AI.make_safe_move()
    if mv is None: return {"move":None,"newly":[],"ai_mines":[c for c in _AI.mines],"lost":False,"won":False,"stuck":False,"revealed_count":len(_REVEALED)} # 关键修改：stuck=False
    if not _FIRST:
        fc = (0, 0)
        if _FIRST_MV_MODE == 1: _GAME.make_safe_first_move(fc)
        elif _FIRST_MV_MODE == 2: _GAME.make_safe_first_move2(fc)
        _FIRST=True
...
def ms_get_analysis():
    ...
    pd, _ = _AI._mine_probabilities()
    nm = _AI.make_random_move()
    return {"probs": pd, "next_move": list(nm) if nm else None}
...
def ms_step():
    ...
    mv=_AI.make_random_move() # 恢复原始逻辑
    ...
    if not _FIRST:
        ...
        mv = fc; _FIRST=True
    results=_GAME.reveal_chain(mv); newly=[]
    if any(v==-1 for v in results.values()): _LOST=True; _REVEALED.add(mv); _REVEALED_N[mv]=-1; return {"move":[mv[0],mv[1]],"newly":[[mv[0],mv[1],-1]],"ai_mines":[c for c in _AI.mines],"lost":True,"won":False,"stuck":False,"revealed_count":len(_REVEALED)}
    _AI.add_knowledge_batch(results)
    for (r,c),n in results.items():
        if (r,c) not in _REVEALED: _REVEALED.add((r,c)); _REVEALED_N[(r,c)]=int(n); newly.append([r,c,int(n)])
    if len(_REVEALED)==_H*_W-_M: _WON=True
