...
const getProbColor = (p) => {
    ...
    const r = Math.round(255), g = Math.round(255 - 55 * c), b = Math.round(255 - 55 * c);
    return `rgba(${r},${g},${b},${0.3 + 0.5 * c})`;
};
...
function applyAnalysisOverlay(d) {
    document.querySelectorAll('.cell.analyzed').forEach(c => { if (!c.classList.contains('open') && !c.classList.contains('mine') && !c.classList.contains('flag')) c.textContent = ''; c.style.backgroundColor = ''; c.classList.remove('analyzed', 'next-move'); });
    for (const [k, v] of Object.entries(d.probs)) {
        const m = k.match(/\((\d+),\s*(\d+)\)/); if (!m) continue;
        const r = parseInt(m[1]), c = parseInt(m[2]);
        if (r < 0 || r >= H || c < 0 || c >= W) continue;
        const e = jsCells[r * W + c];
        if (e && !e.classList.contains('open') && !e.classList.contains('mine') && !e.classList.contains('flag')) {
            e.classList.add('analyzed');
            const p = parseFloat(v); if (isNaN(p)) continue;
            e.style.backgroundColor = getProbColor(p);
            e.textContent = Math.round(p * 100).toString().padStart(2, '0');
            e.style.color = '#000'; e.style.fontWeight = 'normal'; e.style.fontSize = 'calc(var(--cell) * var(--board-cell-scale) * 0.6)';
        }
    }
    if (d.next_move && d.next_move.length === 2) {
        const [nr, nc] = d.next_move;
        if (nr >= 0 && nr < H && nc >= 0 && nc < W) {
            const ne = jsCells[nr * W + nc];
            if (ne && !ne.classList.contains('open') && !ne.classList.contains('mine') && !ne.classList.contains('flag')) {
                ne.classList.add('analyzed', 'next-move');
                ne.style.backgroundColor = 'rgba(144,238,144,0.6)';
            }
        }
    }
}
...
async function stepSolve() {
    if (!api || isStepSolving || !gameStarted) return;
    isStepSolving = true;
    try {
        let rP, r;
        do {
            rP = api.makeSafeMove(); r = rP.toJs(); rP.destroy?.();
            applyStepDelta(r);
            if (r.lost || r.won) { gameStarted = false; return; }
            if (r.move) await new Promise(res => setTimeout(res, 0));
        } while (r.move);
        const sP = api.step(); const s = sP.toJs(); sP.destroy?.();
        applyStepDelta(s);
        if (s.lost || s.won || s.stuck) gameStarted = false;
        
        // --- 新增：调用分析并应用覆盖层 ---
        if (gameStarted) {
            const aP = api.getAnalysis(); const a = aP.toJs(); aP.destroy?.();
            applyAnalysisOverlay(a);
        }
        // --- 新增结束 ---
        
    } finally { isStepSolving = false; }
}
// --- 修改结束 ---
...
// --- 修改后：重命名为 startAllSolve 并增强互斥性 ---
function startAllSolve() {
  // 1. 停止任何正在进行的标准自动求解定时器
  stopSolving(); 
  
  // 2. 停止任何正在进行的 Step Solve 状态
  isStepSolving = false; 

  // 3. 启动全自动求解循环
  const tick = async () => {
    const speed = parseInt(inpSpeed.value, 10);
    await stepOnce(); // 这个函数名暂时保持不变，稍后可选重命名
    if (!solvingTimer) return; // 如果定时器被清除（例如游戏结束），则停止循环
    solvingTimer = setTimeout(tick, speed);
  };
  solvingTimer = setTimeout(tick, 0); // 立即启动第一次迭代
  // setStatus("All Solve running..."); // 可选：更新状态信息
}
// --- 修改结束 ---
