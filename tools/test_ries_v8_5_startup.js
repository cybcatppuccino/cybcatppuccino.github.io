...
for(const token of ['RIES v8.5','id="resultTools"','id="sortConfidenceBtn"','script defer src="ries-script.js"']){
  if(!html.includes(token)) throw new Error(`ries.html is missing expected v8.5 token: ${token}`);
}
if(/<th>formula<\/th>|<th>form \(N\.k\.\#\)<\/th>|<th>q-expansion<\/th>/.test(html)) throw new Error('v8.5 should not have standalone formula/form/q-expansion result headers.');
