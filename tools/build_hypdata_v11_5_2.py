#!/usr/bin/env python3
"""Split the RIES v11.5 hypergeometric database into v11.5.2 level chunks.
...
OUT_STATS = ROOT / 'assets' / 'ries-hypdata-v11_5_2-stats.json'
VERSION = '11.5.2'
...
def main():
    ...
    for stage in (1,2,3):
        ...
        name=f'assets/ries-hypdata-v11_5_2-level{level_for_stage[stage]}.js'
        out=ROOT/name
        js='(function(){\nwindow.RIES_HYPDATA_V1152_CHUNKS=window.RIES_HYPDATA_V1152_CHUNKS||[];\n'
        js+=f'window.RIES_HYPDATA_V1152_CHUNKS[{stage-1}]='
    ...
    stats={
        ...
        'notes':'v11.5.2 splits the pFq database into incremental level4/5/6 chunks. Higher levels load all lower chunks, then compare all loaded H rows against all loaded multipliers, so 2F1/3F2 rows are still tested with stage-2 and stage-3 coefficients.'
    }
