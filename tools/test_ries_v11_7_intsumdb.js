...
(async()=>{
  ...
  assert(rows[0].constantDbSource === 'intsumdb-v11.7', 'source marker mismatch');
  ...
  assert(composed.includes('\\frac{2}{3}') && composed.includes('\\left(') && composed.includes('\\right)') && composed.includes('\\,'), 'multiplier LaTeX should preserve escapes');
  ...
  console.log('PASS RIES v11.7 integral/sum runtime smoke test');
