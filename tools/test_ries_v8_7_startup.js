// RIES v8.7 startup/static-deploy smoke test.
// Run from repository root: node tools/test_ries_v8_7_startup.js
const fs = require('fs');
const vm = require('vm');
...
for(const token of ['RIES v8.7','id="resultTools"','id="sortConfidenceBtn"','script defer src="ries-script.js"']){
  if(!html.includes(token)) throw new Error(`ries.html is missing expected v8.7 token: ${token}`);
}
if(/<th>formula<\/th>|<th>form \(N\.k\.\#\)<\/th>|<th>q-expansion<\/th>/.test(html)) throw new Error('v8.7 should not have standalone formula/form/q-expansion result headers.');
...
if(listenerCount < 7) throw new Error('Expected v8.7 UI listeners, including sort buttons, were not attached.');
console.log('PASS RIES v8.7 startup/static-deploy smoke test');
