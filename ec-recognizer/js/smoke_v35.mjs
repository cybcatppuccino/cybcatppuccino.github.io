...
assert(app.includes("const EC_ATLAS_VERSION = 'v35';"), 'runtime version should be v35');
assert(html.includes('>v35<'), 'HTML version badge should be v35');
...
assert(app.includes('v35 explicitly reuses and continues the integral-point cache'), 'v35 S-integral bridge note missing');
...
${core}
globalThis.__v35 = { parseConductorHint, parseConductorListingQuery, createIntegralSearchState, createSIntegralSearchState, runIntegralSearchState, runSIntegralSearchState, importIntegralSearchIntoSIntegralState, trimSIntegralIntegralOverlap };
`, sandbox);

assert(sandbox.__v35.parseConductorHint('6552.') === 6552, 'trailing-dot conductor hint failed');
assert(sandbox.__v35.parseConductorListingQuery('6552.').N === 6552, 'full conductor listing parser failed');
assert(sandbox.__v35.parseConductorListingQuery('6552.a').prefix === '6552.a', 'conductor class-prefix parser failed');
...
const st = sandbox.__v35.createIntegralSearchState(curve9747d2);
await sandbox.__v35.runIntegralSearchState(st, 700, null);
...
const sst = sandbox.__v35.createSIntegralSearchState(curve9747d2, 2, [2]);
const before = sst.found.length;
sandbox.__v35.importIntegralSearchIntoSIntegralState(sst, st, { seedCap: 64 });
const skipped = sandbox.__v35.trimSIntegralIntegralOverlap(sst, st);
