...
adaptiveAI.importDatabase(JSON.parse(fs.readFileSync(args.db||'ai_data/training_db.json','utf-8')),'v7-eval');try{await wasmRollout.ready;}catch{}const r=rngMulberry(Number(args.seed||70421));const res=[];for(let i=0;i<Number(args.episodes||60);i++)res.push(runEpisode(r,Number(args.seconds||15),1/360,String(args.mix||'v7')));console.log(JSON.stringify(summarize(res),null,2));
