...
export class PendulumController {
  ...
  update(state, params, dt, t) {
    ...
    if (escapeActive) {
      ...
      const useLqr = near.angleNorm < lqrAngleLimit && near.speedNorm < lqrSpeedLimit && Math.abs(supportCenterError(state, params) + 0.45 * state.vx) / Math.max(0.01, supportHalfSpan(params)) < 0.76;
