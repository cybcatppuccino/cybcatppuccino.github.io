...
export class PendulumController {
  ...
  update(state, params, dt, t) {
    ...
    if (escapeActive) {
      ...
      const predictedSupportRatio = Math.abs(supportCenterError(state, params) + 0.42 * state.vx) / Math.max(0.01, supportHalfSpan(params));
      const supportReady = this.target.id !== 3 ? 1 : clamp(1 - Math.abs(state.vx) / 4.2, 0, 1) * clamp((0.92 - predictedSupportRatio) / 0.28, 0, 1);
      if (this.retryTimer > 0) {
        raw = retryPreparationAcceleration(state, this.target, params, this.retryDirection);
        this.plan = [];
      } else if (useLqr && supportReady > 0.72) {
        raw = robustLocalAcceleration(state, this.target, params, this.lqrAcceleration(state, params));
        this.plan = [];
      } else if (useLqr && this.target.id === 3) {
        ...
        const localA = robustLocalAcceleration(state, this.target, params, this.lqrAcceleration(state, params));
        const centerA = centerReturnAcceleration(state, params, 1.18);
        const localBlend = clamp(0.28 + 0.58 * supportReady, 0.28, 0.86);
        raw = localBlend * localA + (1 - localBlend) * (0.76 * planned.acceleration + 0.24 * centerA);
        this.plan = planned.plan;
      } else {
        const state3SearchMode = this.state3WideLeftSearchTimer > 0 ? 2 : (this.state3InitialEdgeBoostTimer > 0 ? 1 : 0);
        const planned = chooseCemAcceleration(state, this.target, params, t, this.plan, state3SearchMode);
