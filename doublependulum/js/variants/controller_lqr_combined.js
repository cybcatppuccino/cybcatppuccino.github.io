...
export class PendulumController {
  ...
  update(state, params, dt, t) {
    ...
    if (escapeActive) {
      ...
      const lqrAngleLimit = this.target.id === 3 && this.localCaptureActive ? 0.82 : (this.target.id === 3 ? (params.maxAcc > 20 ? 0.86 : 0.64) : (this.target.id === 2 ? (params.maxAcc > 20 ? 1.08 : 0.96) : (params.maxAcc > 20 ? 0.72 : 0.62)));
      const lqrSpeedLimit = (this.target.id === 3 && this.localCaptureActive ? (params.maxAcc > 20 ? 8.5 : 6.5) : (this.target.id === 3 ? (params.maxAcc > 20 ? 8.4 : 5.6) : (this.target.id === 2 ? (params.maxAcc > 20 ? 8.5 : 7.4) : (params.maxAcc > 20 ? 5.5 : 4.7)))) * adaptiveEnvelope.lqrSpeedScale;
      const useLqr = near.angleNorm < lqrAngleLimit && near.speedNorm < lqrSpeedLimit && Math.abs(supportCenterError(state, params) + 0.45 * state.vx) / Math.max(0.01, supportHalfSpan(params)) < 0.80;
