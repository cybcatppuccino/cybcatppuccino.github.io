...
assert.match(app, /const ANALYSIS_PAINT_INTERVAL_MS = 500;/, 'v18.1 should cap streamed analysis paints at 500 ms');
assert.doesNotMatch(app, /const important = Boolean\(/, 'important analysis results should not bypass the 500 ms paint throttle');
assert.match(app, /setTimeout\(flushAnalysisPaint, Math\.max\(0, ANALYSIS_PAINT_INTERVAL_MS - elapsed\)\)/, 'pending analysis should render on the next throttled paint');
...
console.log('v18.1 UI throttle and bound-stability tests passed.');
