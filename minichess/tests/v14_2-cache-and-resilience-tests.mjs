...
assert.equal(migrated, null, 'v18.2 should start a fresh cache instead of migrating old v14.1 Orion entries');
assert.equal(cache.get(oldKernelKey), null, 'legacy kernel-suffixed v14.1 keys should not resolve after v18.2 cache reset');
assert.equal(storage.getItem('gardner-analysis-cache-v14_1'), null, 'v18.2 cache load should retire stale v14.1 storage');
assert.ok(storage.getItem('gardner-analysis-cache-v18.2'), 'v18.2 cache should persist into the current v18.2 storage bucket');
...
console.log('v18.2 shared cache reset and protection tests passed.');
