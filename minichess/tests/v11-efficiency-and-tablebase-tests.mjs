...

{
  ...
  assert.equal(cached?.engine, ENGINE_VERSION, 'v15 analysis cache should round-trip current engine identity');
  assert.ok([...storage.keys()].some(key => key.includes('v15')), 'persistent cache key should be versioned to v15');
}

console.log('v15 efficiency and tablebase-cache tests passed.');
