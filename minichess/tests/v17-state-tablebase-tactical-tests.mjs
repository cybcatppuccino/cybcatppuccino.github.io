...
assert.equal(ENGINE_VERSION, 'Orion JS 17');
...
assert.match(app, /GAME_STATE_STORAGE_KEY = 'gardner-current-game-v17'/, 'v17 should persist only the current game tree separately');
assert.match(app, /restoreSavedGameState\(\)/, 'v17 should restore the current game tree on reload');
...
console.log('v17 state, tablebase lazy loading and tactical-safety tests passed.');
