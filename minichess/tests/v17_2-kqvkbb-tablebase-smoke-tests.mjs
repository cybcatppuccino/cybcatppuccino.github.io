...
if (!(await tablebase.init())) {
  console.log('v18.2 KQvKBB tablebase smoke: skipped (no manifest found)');
  process.exit(0);
}
if (!tablebase.exactManifest.tables.KQvKBB) {
  console.log('v18.2 KQvKBB tablebase smoke: skipped (KQvKBB absent)');
  process.exit(0);
}
...
try {
  ...
  console.log(`v18.2 KQvKBB tablebase smoke: skipped (${error?.message || error})`);
  process.exit(0);
}
if (analysis?.tablebase !== true) {
  console.log('v18.2 KQvKBB tablebase smoke: skipped (exact metadata/block files unavailable)');
  process.exit(0);
}
...
if (after?.tablebase !== true) {
  console.log('v18.2 KQvKBB tablebase smoke: skipped after move (exact metadata/block files unavailable)');
  process.exit(0);
}
assert.equal(after.lines[0].move, 'e2d3');
assert.equal(after.lines[0].dtm, 5);
console.log('v18.2 KQvKBB tablebase smoke tests passed.');
