...
const SEARCH_VALUE_KEYS=['id','src','vol','V','B','I','N','ehrN','lw','box','content','primitiveVol','primitiveB','prim','nonPrim','maxEdge','minEdge','maxMult','minMult','sing','edge','mult','smooth','allPrimEdges','kCartier','gorFan','clTorsion','antiK2','fanoIndex','smoothCones','adjDepth','canonicalVertices','centSym','autOrder','autPos','autNeg','nontrivAut','hasReflection','autType','minWidthDirCount','interiorDim','interiorCollinear','reflexiveOrigin','minkowskiDecomp','toricallyMinimalSmooth'];
...
function filterRow(r,F){
  ...
  let mult=rowMultArray(r), maxMult=mult.length?Math.max(...mult):0, minMult=mult.length?Math.min(...mult):0, sing=mult.filter(x=>x>1).length;
  ...
  // v33: legacy metadata may contain the former facet-normal proxy. Active lw filters are checked exactly after the polygon is loaded.
  if(!hasVal(F.lw)&&!cmp(r[MIDX.lw],F.lwOp,F.lw))return false;
...
function exactSearchRequested(F){return ['lw','box','primitiveVol','primitiveB','kCartier','clTorsion','antiK2','fanoIndex','smoothCones','adjDepth','canonicalVertices','autOrder','autPos','autNeg','minWidthDirCount'].some(k=>hasVal(F[k]))||hasVal(F.gorFan)||hasVal(F.nontrivAut)||hasVal(F.hasReflection)||hasVal(F.autType)||hasVal(F.interiorDim)||hasVal(F.interiorCollinear)||hasVal(F.reflexiveOrigin)||hasVal(F.minkowskiDecomp)||hasVal(F.toricallyMinimalSmooth)}
...
async function exactFilterRow(r,F){
  ...
  if(!cmp(s.lw,F.lwOp,F.lw))return false;
  ...
  if(hasVal(F.minWidthDirCount)&&!cmp(minWidthDirectionCount(v),F.minWidthDirCountOp,F.minWidthDirCount))return false;
  if(hasVal(F.interiorDim)&&!boolFilter(interiorPolygonDimensionValue(v),F.interiorDim))return false;
  if(hasVal(F.interiorCollinear)&&!boolFilter(interiorPointsCollinearValue(v)?1:0,F.interiorCollinear))return false;
  if(hasVal(F.reflexiveOrigin)&&!boolFilter(hasReflexiveInteriorOrigin(v)?1:0,F.reflexiveOrigin))return false;
  if(hasVal(F.minkowskiDecomp)&&!boolFilter(isMinkowskiDecomposable(v)?1:0,F.minkowskiDecomp))return false;
  if(hasVal(F.toricallyMinimalSmooth)&&!boolFilter(toricallyMinimalSmoothValue(v)?1:0,F.toricallyMinimalSmooth))return false;
