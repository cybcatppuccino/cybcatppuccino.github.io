...
// Data
// -----------------------------------------------------------------------------
const V4=window.V4_DATA||{}, EXT=window.EXTENDED_DATA||{}, V9=window.V9_DATA||{}, V11=window.V11_DATA||{}, V12=window.V12_DATA||{}, V13=window.V13_DATA||{}, V15=window.V15_DATA||{}, V16=window.V16_DATA||{}, V17=window.V17_DATA||{}, V18=window.V18_DATA||{}, V20=window.V20_DATA||{}, V21=window.V21_DATA||{}, V22=window.V22_DATA||{}, V24=window.V24_DATA||{};
...
function moonshineDetailHTML(name){const m=moonshineGroups[name];if(!m)return'';const symbol=m.symbol,same=moonClassesForSymbol(symbol),adj=moonshineAdjacency[symbol]||{super:[],sub:[]},rels=moonshineRelations.filter(r=>r.sub_symbol===symbol||r.super_symbol===symbol),direct=rels.filter(r=>r.immediate);let x=`<div class="moon-selected"><div class="relation-head">${esc(name)} · moonshine group</div>`;
  ...
  x+=moonshineModularFormsHTML(m,name);
...
function mfSeriesTex(a,maxN=60){if(!Array.isArray(a))return'';const terms=[];for(let n=0;n<=Math.min(maxN,a.length-1);n++){const raw=String(a[n]??'0').trim();if(raw==='0')continue;const q=n===0?'':(n===1?'q':`q^{${n}}`),neg=raw.startsWith('-')&&!/[+].*y|-.+-.+/.test(raw),body=neg?raw.slice(1).trim():raw,coef=(body==='1'&&q)?'':(body==='-1'&&q?'-':gpCoeffTex(body)),term=q?(coef&&coef!=='-'?`\\left(${coef}\\right)${q}`:`${coef}${q}`):gpCoeffTex(raw);terms.push({neg,term,raw})}if(!terms.length)return'0';let out='';for(let i=0;i<terms.length;i++){const t=terms[i];if(i===0)out+=(t.neg?'-':'')+t.term;else out+=t.neg?' - '+t.term:' + '+t.term}return out+`+O(q^{${Math.min(maxN,a.length-1)+1}})`}
...
function crossRelationTex(r){return String(r.phi_tex||'').replace(/\bX\b/g,`T_{${r.moon}}`).replace(/\bY\b/g,`t_{\\rm CP}`)}
function moonCongruenceForCP(label){const a=moonCongruenceRelations.filter(r=>r.cp===label);if(!a.length)return'';let out=`<details class="math-details"><summary>moonshine ↔ congruence algebraic relations · degree ≤ 4 (${a.length})</summary>`;for(const r of a)out+=String.raw`<div class="mathline tiny formula-scroll">\[${crossRelationTex(r)}=0.\]</div><div class="source-note">${esc(r.moon)} · phase 0 · coefficient-certified through u^${r.checked_through||56}</div>`;return out+'<div class="source-note">These are exact integer/rational polynomial certificates from the stored local series; cyclotomic phase-identities already present in the moonshine matching section remain separate.</div></details>'}
function moonCongruenceForMoon(name){const a=moonCongruenceRelations.filter(r=>r.moon===name);if(!a.length)return'';let out=`<details class="math-details"><summary>congruence Hauptmodul relations · degree ≤ 4 (${a.length})</summary>`;for(const r of a)out+=String.raw`<div class="mathline tiny formula-scroll">\[${crossRelationTex(r)}=0,\qquad t_{\\rm CP}=t_{${esc(r.cp)}}.\]</div><div class="source-note">${esc(r.cp)} · coefficient-certified through u^${r.checked_through||56}</div>`;return out+'</details>'}
function modularCurveTheory(r){
  ...
  const g0=standardGamma0Level(r);if(g0)x+=gamma0ModularFormsHTML(g0,lab);
