...
function coverRow(r,rel=null){const can=!!coverTriple(r),verified=r.exact||r.verified,q=rel||(!selectedCover?normalityBetween(r,null):null);return `<button class="cover-row ${can?'exact':''} ${selectedCover?.key===r.key?'selected':''}" data-cover="${esc(r.key)}"><span class="cover-dot">${can?'●':(verified?'◦':'○')}</span><span>${esc(coverLabel(r))}</span><span class="cover-meta">${r.degree??'—'}</span><span class="cover-meta">g=${r.genus}</span>${normalityHTML(q)}</button>`}
...
function subgroupClassTree(g,groups){
  ...
  edges.forEach((e,ei)=>{const A=nodes[e.a],B=nodes[e.b],a=pos.get(A.id),b=pos.get(B.id),hue=(205+53*Math.log2(Math.max(1,e.ratio)))%360,stroke=`hsl(${hue.toFixed(1)} 52% 42%)`,ym=(a.y+b.y)/2+((ei%5)-2)*1.4;svg+=`<path class="sub-edge" style="stroke:${stroke}" marker-end="url(#sub-arrow)" d="M${a.x},${a.y+7} C${a.x},${ym} ${b.x},${ym} ${b.x},${b.y-8}"/>`;if(edges.length<75||e.ratio>2||e.norm?.status==='normal'){const nt=e.norm?.status==='normal'?` · ◁${quotientUnicode(e.norm.quotient_tex)}`:(e.norm?.status==='not_normal'?' · ⋪':'');svg+=`<text class="sub-edge-label" x="${a.x*.43+b.x*.57+3}" y="${ym-3}">${e.ratio}${nt}</text>`}});
  for(const n of nodes){const p=pos.get(n.id);if(n.id==='__gamma__')svg+=`<g><circle class="node selected" cx="${p.x}" cy="${p.y}" r="6.5"/><text text-anchor="middle" x="${p.x}" y="${p.y+18}">Γ</text></g>`;else{const sel=selectedCover?.key===n.rep.key,fill=n.genus===0?'rgba(40,115,76,.10)':'rgba(75,78,92,.07)';svg+=`<g data-cover="${esc(n.rep.key)}" class="graph-hit"><circle class="node ${sel?'selected':''}" style="fill:${fill}" cx="${p.x}" cy="${p.y}" r="${sel?7:5.5}"/><text text-anchor="middle" x="${p.x}" y="${p.y+17}">${n.degree}:${n.genus}</text><title>${esc(coverLabel(n.rep))} · [·]=${n.degree} · g=${n.genus}</title></g>`}}
  svg+='</svg>';const out={svg,nodes,edges};subgroupTreeCache.set(cacheKey,out);return out
}
function subgroupClassDetails(groups){
  let out='<div class="subgroup-class-list">';for(const q of groups){const r=q.rep,ell=(r.elliptic_orders||[]).join(',')||'—',cus=(r.cusp_widths||[]).join(',')||'—',parts=(r.cycle_partitions||triplePartitions(coverTriple(r))||[]).map(x=>'('+x.join(',')+')').join(' / '),vars=q.variants||[],nr=normalityBetween(r,null),nt=nr.status==='normal'?` · ◁ ${quotientUnicode(nr.quotient_tex)}`:(nr.status==='not_normal'?' · ⋪':'');out+=`<details class="subgroup-class"><summary><span>[Γ:H]=${r.degree} · g=${r.genus}${nt}</span><small>${esc(ell)} ; ∞ ${esc(cus)}${vars.length?` · ×${vars.length+1}`:''}</small></summary><div class="class-meta">${esc(parts||'—')}</div><div class="class-actions"><button data-cover="${esc(r.key)}" class="class-cover ${selectedCover?.key===r.key?'selected':''}">H₁</button>${vars.slice(0,36).map((v,i)=>`<button data-cover="${esc(v.key)}" class="class-cover ${selectedCover?.key===v.key?'selected':''}">H${i+2}</button>`).join('')}</div></details>`}return out+'</div>'
}
...
function coversPanel(g){
  ...
  const census=rows.filter(r=>r.census||r.lowIndex);if(census.length){const groups=lowIndexClassGroups(rows),meta=lowIndexMeta[g.id]||{},tree=subgroupClassTree(g,groups),rooted=census.filter(r=>r.conjugacy_class===false).length;out+=`<details class="math-details subgroup-tree-block" open><summary>${tr('lowIndex')} · ${groups.length}</summary><div class="source-note">${census.length} distinct rooted low-[·]=subgroups · ${meta.genus0_classes??groups.filter(q=>q.rep.genus===0).length} genus-0 conjugacy classes${meta.complete_through_index?` · exhaustive through [·]=${meta.complete_through_index}`:''}${meta.verified_nonexhaustive_classes?` · ${meta.verified_nonexhaustive_classes} additionally verified non-exhaustive classes`:''}. Edge K → H means a verified equivariant coset quotient, hence a conjugate of H lies in K.</div><div class="subgroup-tree-scroll">${tree.svg}</div></details><details class="math-details"><summary>${tr('conjugacyClasses')} · ${groups.length}</summary>${subgroupClassDetails(groups)}</details>`;}
...
function commClassGraph(g){
  ...
  for(const rel of edges){const A=pos.get(rel.parent||rel.source),B=pos.get(rel.child||rel.target);if(!A||!B)continue;const ym=(A.y+B.y)/2,t=commRelTriple(rel),nr=t?normalityBetween({key:`commnorm:${rel.key||rel.parent+'|'+rel.child}`,degree:rel.degree||rel.index,triple:t},null):null;let mark='';if(nr?.status==='normal')mark=` · ◁${quotientUnicode(nr.quotient_tex)}`;else if(nr?.status==='not_normal')mark=' · ⋪';svg+=`<path class="edge ${rel.exactGeometry?'strong':'legacy'}" marker-end="url(#comm-arrow)" d="M${A.x},${A.y+8} C${A.x},${ym} ${B.x},${ym} ${B.x},${B.y-10}"/><text class="edge-label" text-anchor="middle" x="${(A.x+B.x)/2+8}" y="${ym-5}">${rel.degree||rel.index}${mark}</text>`}
  for(const m of objects){const q=pos.get(m.id),active=m.id===g.id||m.id===selectedCommPeer;if(!q)continue;svg+=`<g data-comm-peer="${m.id}" class="graph-hit"><circle class="node ${m.triangle?'':'quad-node'} ${active?'selected':''}" cx="${q.x}" cy="${q.y}" r="${active?8:6.5}"/><text text-anchor="middle" x="${q.x}" y="${q.y+23}">${esc(m.label)}</text></g>`}svg+=`<text class="muted-text" x="12" y="18">K → H</text></svg>`;return{svg,objects,edges,levels:byDepth}
}
...
function commRelationDetail(g,G){
  ...
  let out=String.raw`<div class="cover-detail primary"><div class="relation-head">\(\mathcal C(\Gamma)\)</div>`;
...
function commPanel(g){
  ...
  if(qs.length){out+=`<details class="math-details" open><summary>${tr('quadrilaterals')} · ${tr('simultaneous')} (${qs.length})</summary><div class="comm-quad-list">`;for(const q of qs)out+=`<button class="comm-quad-row ${selectedCommPeer===q.id?'selected':''}" data-comm-peer="${esc(q.id)}"><span>\\(${esc(String(q.signature).replace(/inf/g,'\\infty'))}\\)</span><small>[·]=${q.degree} · Δ${esc(byId.get(q.parent)?.signature.replace(/inf/g,'∞')||q.parent)}</small></button>`;out+=`</div><div class="source-note">Every row above has an exact common triangle realization with the current Γ, so clicking compares both fundamental regions without changing Γ.</div></details>`}
  if(all.length){const groups=new Map();for(const q of all){if(!groups.has(q.parent))groups.set(q.parent,[]);groups.get(q.parent).push(q)}out+=`<details class="math-details"><summary>${tr('quadrilaterals')} (${all.length})</summary>`;for(const [pid,arr] of [...groups.entries()].sort((a,b)=>String(byId.get(a[0])?.signature||a[0]).localeCompare(String(byId.get(b[0])?.signature||b[0])))){out+=`<div class="neighbor-title">Δ${esc(byId.get(pid)?.signature.replace(/inf/g,'∞')||pid)} · ${arr.length}</div><div class="comm-quad-list">`;for(const q of arr.sort((a,b)=>a.degree-b.degree||String(a.signature).localeCompare(String(b.signature)))){if(comp.has(q.id))out+=`<button class="comm-quad-row ${selectedCommPeer===q.id?'selected':''}" data-comm-peer="${esc(q.id)}"><span>\\(${esc(String(q.signature).replace(/inf/g,'\\infty'))}\\)</span><small>[·]=${q.degree}</small></button>`;else out+=`<div class="comm-quad-row passive"><span>\\(${esc(String(q.signature).replace(/inf/g,'\\infty'))}\\)</span><small>[·]=${q.degree}</small></div>`}out+='</div>'}out+='<div class="source-note">Passive rows are exact actions but no compatible simultaneous embedding with the currently selected triangle is stored; they are not made clickable rather than asserting a false polygon comparison.</div></details>'}
