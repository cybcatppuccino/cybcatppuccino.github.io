...
function normalizeTexHtml(html){return String(html??'').replace(/\{\}_2F_1/g,'{}_{2}F_{1}').replace(/(^|[^{}])_2F_1/g,'$1{}_{2}F_{1}').replace(/\\Gamma_([01])(?![A-Za-z0-9{])/g,'\\Gamma_{$1}').replace(/\\mathrm\{(SL|GL)\}_([23])(?![A-Za-z0-9{])/g,'\\mathrm{$1}_{$2}').replace(/([A-Za-z])_\\Gamma_\{([01])\}/g,'$1_{\\Gamma_{$2}}').replace(/_\\infty(?![A-Za-z])/g,'_{\\infty}').replace(/\\left\(([^\n]*?)\\right(?!\))/g,'\\left($1\\right)')}
...
function regionVertexData(R){
  ...
  for(let s0=0;s0<(R.reps||[]).length;s0++)for(const A of [R.reps[s0],isoCompose(R.reps[s0],SYS.R[0])])for(let i=0;i<3;i++){
    const z=isoApply(A,SYS.tri[i]),k=pointKey(z);if(!boundaryPts.has(k))continue;const v=upstairsVertexOrder(R,s0,i),width=upstairsCuspWidth(R,s0,i),rep=R.P?permCycleRepresentative(R.P[i],s0):0,classKey=`${prefix}:${i}:${rep}`,old=labels.get(k);
    if(v==='inf'){if(!old||old.order==='inf')labels.set(k,{order:'inf',width:Math.max(width||1,old?.width||1),classKey});continue}
    if(!old||old.order==='inf'||Number(v)<Number(old.order))labels.set(k,{order:v,width:null,classKey})
  }
  const out=[];let regular=0;for(const[k,z]of boundaryPts){const a=labels.get(k)||{order:1,width:null,classKey:`${prefix}:regular:${regular++}`};out.push({z,order:a.order,width:a.width,key:k,classKey:a.classKey,sourceKey:prefix,sourceKind:'cover',center:R.center})}R.vertexData=out;return out
}
function mergedRegionVertexData(regions){
  const merged=new Map();
  for(const R of regions){if(!R)continue;const center=R.center||C();for(const q0 of regionVertexData(R)){const q={...q0,center:q0.center||center,sourceRegion:R},k=pointKey(q.z),ideal=q.order==='inf',old=merged.get(k);
  ...
  for(const q of data){const typ=q.order==='inf'?'inf':String(q.order),ck=q.classKey||`${q.sourceKey||''}:${q.key}`,L=letters.get(`${typ}|${ck}`)||'';q.classLetter=L;q.displayLabel=q.order==='inf'?`${q.width||1}∞${L}`:(Number(q.order)>=2?`${q.order}${L}`:'')}
  return data
}
...
function drawRegionVertexLabels(R,{stroke='rgba(28,28,25,.38)',fill='rgba(249,249,245,.92)',text='rgba(22,22,20,.96)',alpha=1}={}){
  ...
  for(const q of data){const ideal=q.order==='inf';if(!ideal&&Number(q.order)<2)continue;const raw=ideal?idealScreenPoint(q):screenWorld(q.z);if(!ideal&&(!Number.isFinite(raw.x+raw.y)||raw.x<-22||raw.x>W+22||raw.y<-22||raw.y>H+22))continue;const anchor=screenWorld(q.center||C()),pin=ideal?pinCuspToViewport(raw,anchor):{p:raw,off:false,dir:C()},p=pin.p;if(!Number.isFinite(p.x+p.y))continue;const label=q.displayLabel||(ideal?`${q.width||1}∞`:orbifoldOrderText(q.order)),rad=Math.max(9.6,6.7+2.55*label.length);
    ...
    ctx.fillStyle=fill;ctx.beginPath();ctx.arc(p.x,p.y,rad,0,Math.PI*2);ctx.fill();ctx.strokeStyle=(selectedOrbifoldPoint?.classKey&&selectedOrbifoldPoint.classKey===q.classKey)?'rgba(36,73,132,.88)':stroke;ctx.lineWidth=(selectedOrbifoldPoint?.classKey&&selectedOrbifoldPoint.classKey===q.classKey)?2.2:1.25;ctx.stroke();ctx.fillStyle=text;ctx.fillText(label,p.x,p.y+.2);vertexHitTargets.push({x:p.x,y:p.y,r:rad+6,q:{...q,label}})}ctx.restore()
}
...
function drawFrickeCuspLabels(){
  if(!frickeCompare)return;const P=frickeCompare.moonshine?currentMoonshineFordRegion():frickeCompare.plus;if(frickeCompare.moonshine)drawRegionVertexLabels(P,{stroke:'rgba(31,38,52,.52)'});else{const inf=C(1,0);drawStandaloneCusp(inf,1,P.center,{stroke:'rgba(31,38,52,.52)'});if(frickeCompare.gamma0){const zero=icy(C(0,0));drawStandaloneCusp(zero,frickeCompare.N,frickeCompare.gamma0.center,{stroke:'rgba(132,55,53,.45)',fill:'rgba(249,249,245,.90)'})}}
}
...
function moonshineModularFormsHTML(meta,name){
  ...
  let out=`<details class="math-details" open><summary>${full?'modular-form basis on the moonshine group':'modular forms inherited from the eigengroup'} · weights 2,4,6</summary>`;
  if(full){
  ...
  for(const k of ['2','4','6']){
    ...
    out+=`<details class="math-details"><summary>${full?`M_${k}(Γ_${esc(name)})`:`E_${esc(name)}-invariant part of M_${k}(Γ_${esc(name)})`} · dimension ${d}</summary>`;
...
function moonshineDetailHTML(name){const m=moonshineGroups[name];if(!m)return'';const symbol=m.symbol,same=moonClassesForSymbol(symbol),adj=moonshineAdjacency[symbol]||{super:[],sub:[]},rels=moonshineRelations.filter(r=>r.sub_symbol===symbol||r.super_symbol===symbol),direct=rels.filter(r=>r.immediate);let x=`<div class="moon-selected"><div class="relation-head">${esc(name)} · moonshine group</div>`;
  ...
  const gm=Object.entries(moonshine.gamma0Matches||{}).filter(([,q])=>q.monster_class===name);if(gm.length){x+=`<div class="source-note">Certified standard congruence match: ${gm.map(([cp,q])=>`${cp} ↔ Γ0(${q.level})`).join(' · ')}</div>`;const Ns=[...new Set(gm.map(([,q])=>Number(q.level)))];for(const N of Ns)x+=gamma0ModularFormsHTML(N,name)}x+=moonCongruenceForMoon(name);
  return x+'</div>'
}
...
function gamma0ModularFormsHTML(N,context=''){
  const D=modularFormsGamma0[String(N)];if(!D)return'';let out=`<details class="math-details"><summary>modular forms on Γ0(${N}) · weights 2,4,6</summary><div class="source-note">Exact PARI/GP basis data through q^60. Every generator is written explicitly; Hecke eigenforms below refer to the newspace.</div>`;
  for(const k of ['2','4','6']){const z=D[k];if(!z)continue;out+=`<details class="math-details"><summary>M_${k}(Γ0(${N})) · ${z.basis.length} basis vector${z.basis.length===1?'':'s'}</summary>`;z.basis.forEach((a,i)=>{out+=String.raw`<div class="mathline tiny formula-scroll">\[f_{${k},${i+1}}(q)=${mfSeriesTex(a,60)}.\]</div>`});if(z.eigen?.length){out+=`<div class="relation-head">newspace Hecke eigenbasis · ${z.eigen.length}</div>`;if(z.fields&&z.fields!=='[]')out+=String.raw`<div class="mathline tiny formula-scroll">\[\text{PARI coefficient fields: }${gpCoeffTex(z.fields)}.\]</div>`;z.eigen.forEach((a,i)=>{out+=String.raw`<div class="mathline tiny formula-scroll">\[g_{${k},${i+1}}(q)=${mfSeriesTex(a,60)}.\]</div>`})}out+='</details>'}return out+'</details>'
}
function cpGenus0ModularFormsHTML(label){
  const D=cpGenus0ModularForms[label];if(!D)return'';let out=`<details class="math-details"><summary>direct genus-zero modular-form basis · weights 2,4,6</summary><div class="source-note">Reconstructed on the actual CP genus-zero group from its cusp-normalized Hauptmodul and the exact Belyi branch divisor. The basis is computed in the regular local coordinate s=1/t, so the displayed u^60 coefficients do not rely on cancellation of separately truncated Laurent poles.</div>`;out+=String.raw`<div class="mathline tiny formula-scroll">\[j=\frac{${D.j_num_tex}}{${D.j_den_tex}},\qquad u=q^{1/${D.cusp_width}},\qquad ${D.field_tex||'\mathbb Q'}.\]</div>`;
  for(const k of ['2','4','6']){const z=D.weights?.[k];if(!z)continue;out+=`<details class="math-details"><summary>M_${k}(Γ_CP(${esc(label)})) · dimension ${z.dimension}</summary>`;if(!z.basis?.length)out+=String.raw`<div class="mathline tiny">\[M_${k}=0.\]</div>`;else z.basis.forEach((a,i)=>{out+=String.raw`<div class="mathline tiny formula-scroll">\[F_{${k},${i+1}}(u)=${mfSeriesTex(a,60,'u')}.\]</div>`});out+='</details>'}return out+'</details>'
}
function gamma1Level(r){const m=String(r?.row?.name||'').match(/Γ\s*1\s*\((\d+)\)/);return m?Number(m[1]):null}
function gamma1CharacterFormsHTML(N){const D=gamma1CharacterForms[String(N)];if(!D)return'';let out=`<details class="math-details"><summary>Γ1(${N}) character decomposition · weights 2,4,6</summary><div class="source-note">Exact PARI/GP decomposition into M_k(Γ0(${N}),χ) over Galois-orbit representatives of Dirichlet characters; every displayed basis vector is expanded through q^60.</div>`;for(const k of ['2','4','6']){const blocks=D[k]||[];out+=`<details class="math-details"><summary>weight ${k} · ${blocks.length} character orbit${blocks.length===1?'':'s'}</summary>`;blocks.forEach((z,j)=>{out+=`<details class="math-details"><summary>χ${j+1} · order ${z.order} · dimension ${z.dimension}</summary><div class="source-note">character ${esc(z.character)} · coefficient field ${esc(z.field)}</div>`;(z.basis||[]).forEach((a,i)=>out+=String.raw`<div class="mathline tiny formula-scroll">\[f_{${k},${j+1},${i+1}}(q)=${mfSeriesTex(a,60)}.\]</div>`);out+='</details>'});out+='</details>'}return out+'</details>'}
...
function cpSelectedPointHTML(r){const q=selectedOrbifoldPoint,D=cpGenus0ModularForms[r?.label];if(!q||!D||q.sourceKind!=='cover')return'';const ideal=q.order==='inf';if(ideal&&absv(sub(q.z,C(1)))<2e-5)return String.raw`<div class="divider"></div><div class="relation-head">selected orbifold point · ${esc(q.label||'∞')}</div><div class="mathline small formula-scroll">\[t(P)=\infty.\]</div><div class="source-note">The selected representative is the normalized pole cusp.</div>`;const kind=ideal?'inf':String(q.order),roots=flattenVertexRoots(D,kind,ideal?q.width:null);if(!roots.length)return'';let pick=null;if(roots.length===1)pick=roots[0];else{const val=cpHauptNumeric(D,q.z,ideal?.92:1);if(val)pick=roots.map(z=>({...z,dist:Math.hypot(val.x-z.re,val.y-z.im)})).sort((a,b)=>a.dist-b.dist)[0]}
  ...
  return String.raw`<div class="divider"></div><div class="relation-head">selected orbifold point · ${esc(q.label||'')}</div><div class="mathline small formula-scroll">\[t(P)=${exact}.\]</div><div class="source-note">Exact coordinate from the reconstructed j-fibre (${ideal?'cusp':'elliptic order '+q.order}); numerical branch ≈ ${esc(approx)}.</div>`
}
...
function moonshineSelectedPointHTML(name){
  ...
  if(q.order==='inf'&&absv(sub(q.z,C(1)))<2e-5)return String.raw`<div class="divider"></div><div class="relation-head">selected orbifold point · ${esc(q.label||'∞')}</div><div class="mathline small">\[T_{${esc(name)}}(P)=\infty.\]</div><div class="source-note">Normalized pole cusp.</div>`;
  if(D){const roots=moonJPointRoots(D,q.order);let pick=null;if(roots.length===1)pick=roots[0];else if(q.order!=='inf'&&roots.length){const z=moonHauptNumeric(name,q.z);if(z)pick=roots.map(r=>({...r,dist:Math.hypot(z.x-r.re,z.y-r.im)})).sort((a,b)=>a.dist-b.dist)[0]}
    if(pick){const f=pick.factor,exact=pick.exact_tex||`\\alpha\\quad\\text{with}\\quad ${String(f.poly_tex||'').replace(/Y/g,'\\alpha')}=0`,approx=`${Number(pick.re).toPrecision(9)}${pick.im>=0?'+':'−'}${Math.abs(Number(pick.im)).toPrecision(7)}i`;return String.raw`<div class="divider"></div><div class="relation-head">selected orbifold point · ${esc(q.label||'')}</div><div class="mathline small formula-scroll">\[T_{${esc(name)}}(P)=${exact}.\]</div><div class="source-note">Exact coordinate from the low-level j/Hauptmodul correspondence; numerical branch ≈ ${esc(approx)}.</div>`}
    if(q.order==='inf'&&(D.points?.finite_cusps||[]).length){const polys=D.points.finite_cusps.map(f=>String(f.poly_tex||'').replace(/Y/g,`T_{${esc(name)}}`)).filter(Boolean);if(polys.length)return String.raw`<div class="divider"></div><div class="relation-head">selected cusp · ${esc(q.label||'∞')}</div><div class="mathline tiny formula-scroll">\[T_{${esc(name)}}(P)\\text{ is a root of }${polys.map(x=>'('+x+')').join('\\,')}.\]</div><div class="source-note">The exact finite-cusp fibre is known; this boundary representative is not numerically assigned to a particular conjugate root from a truncated q-series.</div>`}}
  const z=moonHauptNumeric(name,q.z);if(!z)return'';let tex=null;if(Math.abs(z.y)<2e-7)tex=exactTex(z.x);const numeric=`${Number(z.x).toPrecision(10)}${z.y>=0?'+':'−'}${Math.abs(Number(z.y)).toPrecision(8)}i`;return String.raw`<div class="divider"></div><div class="relation-head">selected orbifold point · ${esc(q.label||'')}</div>${tex?`<div class="mathline small formula-scroll">\[T_{${esc(name)}}(P)=${tex}.\]</div>`:''}<div class="source-note">q^60 evaluation: ${esc(numeric)}${tex?' · recognized as the displayed low-degree value':' · no stronger exact recognition is asserted for this point'}.</div>`
}
...
function crossRelationTex(r){return String(r.phi_tex||'').replace(/\bX\b/g,relationMoonArgTex(r)).replace(/\bY\b/g,`t_{\\rm CP}(u)`)}
...
function moonCongruenceForCP(label){const a=moonCongruenceRelations.filter(r=>r.cp===label);if(!a.length)return'';let out=`<details class="math-details"><summary>moonshine ↔ congruence algebraic relations · total degree ≤ 4 (${a.length})</summary>`;for(const r of a)out+=String.raw`<div class="mathline tiny formula-scroll">\[${crossRelationTex(r)}=0.\]</div><div class="source-note">${esc(r.moon)} · ${relationTransformNote(r)} · coefficient-certified through u^${r.checked_through||56}</div>`;return out+'<div class="source-note">v25 searches the stored local expansions with q→q²,q³ and roots of unity of exact orders 2,3,4,6 in addition to the untwisted identities; only coefficient-certified rational polynomial relations are displayed.</div></details>'}
function moonCongruenceForMoon(name){const a=moonCongruenceRelations.filter(r=>r.moon===name);if(!a.length)return'';let out=`<details class="math-details"><summary>congruence Hauptmodul relations · total degree ≤ 4 (${a.length})</summary>`;for(const r of a)out+=String.raw`<div class="mathline tiny formula-scroll">\[${crossRelationTex(r)}=0,\qquad t_{\\rm CP}=t_{${esc(r.cp)}}.\]</div><div class="source-note">${esc(r.cp)} · ${relationTransformNote(r)} · coefficient-certified through u^${r.checked_through||56}</div>`;return out+'</details>'}
...
function coverDetail(r){
  ...
  if(jr?.status==='jacobian_resolved_curve_torsor_not_asserted'&&jr.jacobian_j_invariant){x+=`<div class="divider"></div><div class="relation-head">genus-1 Jacobian</div><div class="mathline small">\\[j(\\operatorname{Jac}X_H)=${ratTex(jr.jacobian_j_invariant)},\qquad N_E=${jr.jacobian_conductor??'—'}.\]</div><div class="source-note">Exact Jacobian identification from the supplied genus-one curve record; the torsor itself is not asserted to carry the same chosen origin.</div>`}
  if(r.higher){
...
function exactHGChainHTML(path,r,g,h){
  ...
  let out=`<div class="relation-head">eliminated 2F1 pullback chain · ${n} exact step${n>1?'s':''}</div>`;
  ...
  if(c?.factor_tex){
    out+=String.raw`<div class="relation-head">endpoint resultant</div><div class="mathline small formula-scroll">\[\boxed{\Phi_{\Gamma,\Gamma'}(x,y)=${c.factor_tex}=0}.\]</div>`;
    out+=`<div class="source-note">Intermediate coordinates z₁,…,z${n-1} were eliminated by iterated resultants; endpoint bidegree (${c.degree_x}, ${c.degree_y}).${c.factor_tex!==c.phi_tex?' The resultant is displayed in factored form so its algebraic components remain visible.':''}</div>`;
    if(c.scalar_composable&&c.quotient_tex){
      out+=String.raw`<div class="relation-head">composed scalar quotient</div><div class="mathline tiny formula-scroll">\[F_{\Gamma}^{\rm loc}(x):=${c.start_solution_tex},\qquad F_{\Gamma'}^{\rm loc}(y):=${c.end_solution_tex}.\]</div>`;
      out+=String.raw`<div class="mathline formula-scroll pullback-ratio">\[\boxed{\frac{F_{\Gamma}^{\rm loc}(x)}{F_{\Gamma'}^{\rm loc}(y)}=${c.quotient_tex}},\qquad z_0=x,\ z_${n}=y.\]</div>`;
      out+=`<div class="source-note">Here every intermediate local 2F1 solution agrees literally on the two adjacent verified edges, so the scalar factors cancel and compose. The remaining zᵢ are algebraic functions on the endpoint curve cut out by the displayed step equations and endpoint resultant.</div>`
    }else{
...
function noncompactTheory(g){
  ...
  const ff=noncompactFourier[g.id]||{},forms=ff.forms||[],expandedBases=noncompactGeneratorSeries[g.id]||[];
  const basisRow=f=>{const e=expandedBases.find(z=>Number(z.weight)===Number(f.weight));let z=String.raw`<div class="relation-head">\(M_{${f.weight}}(\Gamma)\) · ${Number(f.degree)+1} generator${Number(f.degree)===0?'':'s'}</div>`;if(e?.basis?.length){for(const b of e.basis)z+=String.raw`<div class="mathline tiny formula-scroll">\[f_{${f.weight},${b.ell}}(Q):=f_{${f.weight},\Gamma}(Q)J_\Gamma^{${b.ell}}=${b.series}.\]</div>`;z+=`<div class="source-note">These are the individual generators of the recorded span, obtained by exact truncated Laurent-series multiplication with the stored normalized Hauptmodul. No range notation is used in place of the basis.</div>`}else{z+=String.raw`<div class="mathline tiny formula-scroll">\[f_{${f.weight},\Gamma}(Q)=${f.series}.\]</div><div class="source-note">The stored seed begins beyond the available Fourier truncation, so additional powers of J are not fabricated here.</div>`}return z};
  let fseries='';if(forms.length){fseries='<div class="divider"></div><div class="relation-head">canonical holomorphic q-series · explicit bases</div>';for(const f of forms.slice(0,4))fseries+=basisRow(f);if(forms.length>4){fseries+='<details class="math-details"><summary>more weights</summary>';for(const f of forms.slice(4))fseries+=basisRow(f);fseries+='</details>'}}
  ...
  return `<details class="math-details nc-theory" open><summary>cusp / mirror / modular forms</summary><div class="eq-grid compact"><div class="eq-key">realization</div><div class="eq-val">\\(${d.realization}\\)</div><div class="eq-key">cusp coordinate</div><div class="eq-val">\\(Q=${d.Q},\\ q=e^{2\\pi i\\tau}\\)</div></div><div class="mathline small formula-scroll">\\[${d.period}.\\]</div><div class="source-note">Here \\(z=(1-J_\\Gamma)^{-1}=x^{-1}\\). The mirror map and its inverse use this same normalized cusp coordinate.</div><div class="mathline tiny formula-scroll">\\[${d.J}.\\]</div><div class="mathline tiny formula-scroll">\\[${zser}.\\]</div><div class="mathline tiny formula-scroll">\\[${mir}.\\]</div>${fseries}<div class="divider"></div><div class="mathline tiny formula-scroll">\\[${d.ring}.\\]</div><div class="space-head"><span>weight</span><span>dim M</span><span>dim S</span></div><div class="space-table">${rows}</div><div class="source-note">${ncPeers} exact scalar \\({}_{2}F_{1}\\) pullback(s) to other non-compact arithmetic triangle groups are available above. The Fourier forms are the canonical triangle forms recovered from the normalized Hauptmodul; provisional or generic forms are not substituted. Koike's manuscript linked from OEIS A004016 and Doran–Gannon–Movasati–Shokri are used as the non-compact arithmetic normalization references.</div>${referenceLinks()}</details>`
}
function uniformPanel(g){
  ...
  out+=`<div class="source-note">v25 relation closure: all 201 recorded unordered relations among the 85 arithmetic triangle groups are surfaced. Every 64 multi-step verified 2F1 chain now has an endpoint resultant in x,y; 25 of those chains also admit a literal scalar quotient because every intermediate local/Kummer solution cancels. Other chains keep the exact endpoint algebraic relation without inventing a scalar gauge.</div>`;
