...
// Poseidon-only wider-tree profile.  Orion's original fallback worker is not changed.
// The numbers below deliberately make selective pruning and LMR less eager so
// nnue_policy2_best can examine more plausible alternatives before cutting.
const POSEIDON_WIDE_TREE = Object.freeze({
  rfpMaxDepth: 2,
  rfpMinEmpty: 12,
  rfpMinLegal: 3,
  futilityMaxDepth: 1,
  futilityMinSearched: 4,
  futilityMinEmpty: 12,
  lmrMinDepth: 5,
  lmrMinSearched: 5,
  lmrMinEmpty: 12,
  lmrMinOpponentReplies: 3,
  tacticalExtensionMaxPly: 18
});
const DIRS = [
  [-1, -1], [0, -1], [1, -1],
  [-1, 0],           [1, 0],
...
const poseidonStaticCache = new Map();
let activeNNUE = null;
let poseidonModel = null;
let poseidonModelPromise = null;
let poseidonModelFailed = false;
...
const NNUE_EDGE_EXCLUDING_CORNERS = new Set(Array.from({ length: CELL_COUNT }, (_, i) => {
  const x = i % BOARD_SIZE;
  const y = Math.floor(i / BOARD_SIZE);
  return (x === 0 || x === BOARD_SIZE - 1 || y === 0 || y === BOARD_SIZE - 1) && !CORNERS.has(i) ? i : -1;
}).filter((i) => i >= 0));

class PoseidonNNUE {
  constructor(weights) {
    this.scoreScale = Number(weights.score_scale || 36);
    this.accumDim = Number(weights.accum_dim || 128);
    this.hiddenDim = Number(weights.hidden_dim || 64);
    this.embed = flattenFloat32(weights.embed);
    this.accBias = Float32Array.from(weights.acc_bias || []);
    this.fc1 = flattenFloat32(weights.fc1_weight);
    this.fc1Bias = Float32Array.from(weights.fc1_bias || []);
    this.fc2 = Float32Array.from((weights.fc2_weight && weights.fc2_weight[0]) || []);
    this.fc2Bias = Number((weights.fc2_bias && weights.fc2_bias[0]) || 0);
    this.acc = new Float32Array(this.accumDim);
    this.hidden = new Float32Array(this.hiddenDim);
    this.cache = new Map();
  }

  evaluatePerspective(b, side) {
    ...
    const feats = [];
    let xCount = 0, oCount = 0, empty = 0;
    let xCorners = 0, oCorners = 0, xEdges = 0, oEdges = 0;
    for (let i = 0; i < CELL_COUNT; i += 1) {
      const cell = b[i];
      if (cell === side) {
        feats.push(NNUE_OFF.X_PIECE + i);
        xCount += 1;
        if (CORNERS.has(i)) xCorners += 1;
        else if (NNUE_EDGE_EXCLUDING_CORNERS.has(i)) xEdges += 1;
      } else if (cell === opp) {
        feats.push(NNUE_OFF.O_PIECE + i);
        oCount += 1;
        if (CORNERS.has(i)) oCorners += 1;
        else if (NNUE_EDGE_EXCLUDING_CORNERS.has(i)) oEdges += 1;
      } else {
        empty += 1;
      }
    }
    const xMob = legalMovesFor(b, side).length;
    const oMob = legalMovesFor(b, opp).length;
    feats.push(
      NNUE_OFF.X_COUNT + clampBucket(xCount, 36),
      NNUE_OFF.O_COUNT + clampBucket(oCount, 36),
      NNUE_OFF.EMPTY_COUNT + clampBucket(empty, 36),
      NNUE_OFF.X_MOBILITY + clampBucket(xMob, 36),
      NNUE_OFF.O_MOBILITY + clampBucket(oMob, 36),
      NNUE_OFF.X_CORNERS + clampBucket(xCorners, 4),
      NNUE_OFF.O_CORNERS + clampBucket(oCorners, 4),
      NNUE_OFF.X_EDGES + clampBucket(xEdges, 20),
      NNUE_OFF.O_EDGES + clampBucket(oEdges, 20)
    );

    const A = this.accumDim;
    const H = this.hiddenDim;
    const acc = this.acc;
    const hidden = this.hidden;
    for (let j = 0; j < A; j += 1) acc[j] = this.accBias[j];
    for (const fid of feats) {
      const row = fid * A;
      for (let j = 0; j < A; j += 1) acc[j] += this.embed[row + j];
    }
    for (let j = 0; j < A; j += 1) acc[j] = crelu(acc[j]);
    for (let i = 0; i < H; i += 1) {
      let s = this.fc1Bias[i];
      const row = i * A;
      for (let j = 0; j < A; j += 1) s += this.fc1[row + j] * acc[j];
      hidden[i] = crelu(s);
    }
    let y = this.fc2Bias;
    for (let i = 0; i < H; i += 1) y += this.fc2[i] * hidden[i];
    const value = Math.tanh(y) * this.scoreScale;
    if (this.cache.size > 300000) this.cache.clear();
    this.cache.set(key, value);
    return value;
  }

  evaluateWhite(b) {
    return this.evaluatePerspective(b, 'white');
  }

  evaluateMovePerspective(b, side, move) {
    const nb = applyMoveToBoard(b, side, move);
    return this.evaluatePerspective(nb, side);
  }
}

function flattenFloat32(matrix) {
  if (matrix instanceof Float32Array) return matrix;
  const rows = Array.isArray(matrix) ? matrix : [];
  let total = 0;
  for (const row of rows) total += row.length || 0;
...
function crelu(x) { return x <= 0 ? 0 : (x >= 1 ? 1 : x); }
function clampBucket(n, maxValue) {
  n = Math.trunc(n);
  if (n < 0) return 0;
  if (n > maxValue) return maxValue;
  return n;
}
function getPoseidonModel() {
  if (poseidonModel || poseidonModelFailed) return Promise.resolve(poseidonModel);
  if (!poseidonModelPromise) {
    const url = new URL('nnue_policy2_best.json', self.location.href).href;
    poseidonModelPromise = fetch(url)
      .then((res) => {
        if (!res.ok) throw new Error(`NNUE weights not found: ${res.status}`);
        return res.json();
      })
      .then((weights) => {
        poseidonModel = new PoseidonNNUE(weights);
        return poseidonModel;
      })
      .catch((error) => {
        poseidonModelFailed = true;
        console.warn('Poseidon NNUE disabled; falling back to pattern evaluation.', error);
        return null;
      });
  }
...
self.onmessage = (event) => {
  ...
  getPoseidonModel()
    .then((model) => {
      if (localToken !== activeToken) return null;
      searcher.nnue = model;
      activeNNUE = model;
      return searcher.run();
    })
...
class FallbackSearcher {
    ...
    if (!legal.length) {
      for (let depth = 1; depth <= this.hardMaxDepth; depth += 1) {
        ...
        this.bestResult = this.decorate({
          ...
          evalWhite,
          bestMove: PASS,
          source: depth >= this.hardMaxDepth ? 'Poseidon NNUE solved' : 'Poseidon NNUE iterative',
          cache: 'Fallback',
          depth,
  ...
  rootSearch(depth, alphaStart = -INF, betaStart = INF) {
    ...
    for (const move of moves) {
      ...
      lines.push({ move, evalWhite, source: this.fullWidthSearch ? 'Poseidon NNUE solved' : 'Poseidon NNUE iterative', pv: [move, ...childPv] });
      if (score > bestScore) {
        bestScore = score;
        bestMove = move;
      }
  ...
  negamax(b, side, depth, alpha, beta, passCount, ply, outPv, pvNode, previousMove = null) {
    ...
    // Stockfish-style reverse futility pruning.  In the wider Poseidon profile
    // this is intentionally later and more conservative than before: only shallow,
    // clearly winning non-PV nodes with several legal replies are cut.
    if (!this.fullWidthSearch && !pvNode && depth <= POSEIDON_WIDE_TREE.rfpMaxDepth && c.empty > POSEIDON_WIDE_TREE.rfpMinEmpty && legalNow.length >= POSEIDON_WIDE_TREE.rfpMinLegal) {
      const rfpMargin = 2.25 + depth * 0.95;
      if (staticValue - rfpMargin >= beta) {
        return staticValue - rfpMargin * 0.25;
      }
    }
    const preferred = [];
    ...
    for (const move of moves) {
      ...
      // Wider-tree futility: do not prune depth-2 alternatives anymore, and only
      // skip late, quiet, non-tactical frontier moves at the shallowest horizon.
      if (!this.fullWidthSearch && !pvNode && depth <= POSEIDON_WIDE_TREE.futilityMaxDepth && searched >= POSEIDON_WIDE_TREE.futilityMinSearched && !tactical && empty > POSEIDON_WIDE_TREE.futilityMinEmpty) {
        const margin = 1.85;
      ...
      if (!this.fullWidthSearch && depth >= 2 && ply <= POSEIDON_WIDE_TREE.tacticalExtensionMaxPly && (CORNERS.has(move) || oppMobility === 0)) {
        // Critical Othello moves are rare and often decide access/parity.  Give
        // corners and forcing-pass moves one extra ply instead of reducing them.
        childDepth += 1;
      }
      if (!this.fullWidthSearch && !pvNode && depth >= POSEIDON_WIDE_TREE.lmrMinDepth && searched >= POSEIDON_WIDE_TREE.lmrMinSearched && !tactical && empty > POSEIDON_WIDE_TREE.lmrMinEmpty && oppMobility >= POSEIDON_WIDE_TREE.lmrMinOpponentReplies) {
  ...
  decorate(result) {
    ...
    return {
      ...
      elapsedMs: elapsed,
      nnue: activeNNUE ? 'nnue_policy2_best' : 'pattern-fallback',
      source: result.source || 'Poseidon NNUE iterative',
      cache: 'Fallback'
    };
  }
...
function moveOrderingScore(b, side, move) {
  ...
  if (activeNNUE) score += activeNNUE.evaluateMovePerspective(b, side, move) * 1500;
...
function patternHeuristicEvalWhite(b) {
  const key = boardKey(b);
...
function heuristicEvalWhite(b) {
  const c = countsOf(b);
  if (c.empty === 0 || (legalMovesFor(b, 'black').length === 0 && legalMovesFor(b, 'white').length === 0)) return finalWhiteDiff(b);
  const key = `${boardKey(b)}|poseidon:${activeNNUE ? 'nnue_policy2_best' : 'pattern'}`;
  const hit = poseidonStaticCache.get(key);
  if (hit !== undefined) return hit;
  const pattern = patternHeuristicEvalWhite(b);
  if (!activeNNUE) return pattern;
  const nnueWhite = activeNNUE.evaluateWhite(b);
  const empty = c.empty;
  // Trust NNUE most in opening/midgame policy ordering, then blend more exact
  // handcrafted parity/material knowledge as the 6x6 endgame approaches.
  const nnueWeight = empty <= 6 ? 0.55 : empty <= 12 ? 0.68 : empty <= 22 ? 0.80 : 0.86;
  const mixed = clamp(nnueWhite * nnueWeight + pattern * (1 - nnueWeight), -44, 44);
  if (poseidonStaticCache.size > 300000) poseidonStaticCache.clear();
  poseidonStaticCache.set(key, mixed);
  return mixed;
}
...
function aspirationWindow(depth, previousScore, empty) {
  ...
  // Wider windows reduce fail-high/fail-low re-search churn now that Poseidon
  // searches more quiet alternatives at full depth.
  if (empty <= 10) return 6.0;
  if (empty <= 18) return 5.0;
  return 4.2 + Math.min(4.8, depth * 0.18 + Math.abs(previousScore) * 0.05);
}
